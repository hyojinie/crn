...
fclose(fileO);
%%
file = fopen([test_path '/' sv_binary_file_name '_16384.bin'],'r');
cat_ref = fread(file, [4096*4, Inf], 'float=>single');
fclose(file);
cat_ref = cat_ref';
...
query_ref = fread(file, [4096*8, Inf], 'float=>single');
fclose(file);
query_ref = query_ref';

query_ref = query_ref*Utrc;
query_ref = bsxfun(@plus, query_ref, -Utmux(:,1:4096*4));
file = fopen([test_path '/' q3_binary_file_name '_16384.bin'],'w');
...
save([test_path '/' 'test_w_pcaR_16384'],'ret_res','plot_res');