...
DataTransformer<Dtype>::DataTransformer(const TransformationParameter& param,
  ...
  if (eigen_val_size > 0) {
	  ...
	  LOG(INFO) << "Color Augmentation to be used........";
  }
}
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  if ( has_eigen_values) { //phase_ == TRAIN &&
	  ...
	  // fill relight_alpha with random numbers bassed on the filler 
	  // if gaussian, set the std as 0.2 ~ 0.3
	  Blob<Dtype> relight_alpha(1, 1, 1, datum_channels);
	  relight_filler_->Fill(&relight_alpha);

	  // multiply eighen values in the blob relight_alpha
	  for (int c = 0; c < datum_channels; ++c) {
		  relight_alpha.mutable_cpu_data()[c] *= eigen_values_[c];
	  }

	  // reshape blob relight_
	  relight_.Reshape(1, 1, 1, datum_channels);
	  // multiply eigen_vectors and eigen values (relight_alpha), 
	  // then store it to relight_
	  // eigen vectors should be stored in row major 
	  // as it will read as row major, then do transpose to multiply
