...
bool Net<Dtype>::ForwardIsAllowed(int i) {
	...
	int count_bottom_num = 0;
	for (int b = 0; b < bottom_vecs_[i].size(); ++b) {
		LOG(INFO) << i << "-th layer " << b << "-th bottom: " << bottom_vecs_[i][b]->num();
		count_bottom_num += bottom_vecs_[i][b]->num();
	}
	LOG(INFO) << i << "-th layer count bottom: " << count_bottom_num;
	if (count_bottom_num == 0){
			// if a bottom has num == 0, deny the forward and reshape a top
			// to num = 0 to deny the forward of subsequent layers
			for (int t = 0; t < top_vecs_[i].size(); ++t) {
					top_vecs_[i][t]->Reshape(0,
						top_vecs_[i][t]->channels(),
						top_vecs_[i][t]->height(),
						top_vecs_[i][t]->width());
			}
